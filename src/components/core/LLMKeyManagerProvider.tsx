...
export const LLMKeyManagerProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    const unsubscribeValidator = validatorService.subscribe((event) => {
      // Update events list
    ...
    const unsubscribeAvailability = availabilityManager.onUpdate(() => {
      refreshKeys();
    });
  ...
  useEffect(() => {
    ...
    vaultService.unlock().then(async () => {
      ...
      // Initialize availability cache from DB
      const { availabilityCache } = await import("../../services/availability/availability.cache");
