...
const PROVIDER_CIRCUIT_CONFIGS: Partial<Record<AIProviderId, Partial<CircuitBreakerConfig>>> = {
    openai: {
...
class SafetyGuard {
    ...
    setForcedFallback(model: string, provider?: AIProviderId): void {
        ...
        console.warn(`[SafetyGuard] 🎯 Forced fallback: ${model}${provider ? ` (${provider})` : ''}`);
    }
    ...
    private getProviderCircuit(providerId: AIProviderId): CircuitBreakerState {
        if (!this.state.providerCircuitBreakers.has(providerId)) {
            this.state.providerCircuitBreakers.set(providerId, this.createCircuitState());
        }
    ...
    private getCircuitConfig(providerId?: AIProviderId): CircuitBreakerConfig {
        if (providerId && PROVIDER_CIRCUIT_CONFIGS[providerId]) {
            return { ...DEFAULT_CIRCUIT_CONFIG, ...PROVIDER_CIRCUIT_CONFIGS[providerId] };
        }
    ...
    private recordFailure(
        ...
        if (circuit.state === 'OPEN') {
            if (circuit.openedAt && now - circuit.openedAt > config.cooldownMs) {
                ...
                circuit.successes = 0;
                console.log(`[SafetyGuard] 🔄 Circuit ${label} -> HALF_OPEN (testing recovery)`);
        ...
        if (circuit.state === 'HALF_OPEN') {
            ...
            console.warn(`[SafetyGuard] ⛔ Circuit ${label} -> OPEN (recovery failed)`);
        ...
        if (circuit.failureHistory.length >= config.failureThreshold) {
            circuit.state = 'OPEN';
            circuit.openedAt = now;
            console.warn(`[SafetyGuard] ⛔ Circuit ${label} -> OPEN (${config.failureThreshold} failures in ${config.failureWindowMs / 1000}s)`);
            this.emit({ type: 'CIRCUIT_OPENED', label, reason: `${config.failureThreshold} failures` });
        }
     ...
     */
    private recordSuccess(circuit: CircuitBreakerState, label: string): CircuitState {
        const now = Date.now();
    ...
    shouldAllowRequest(keyId: string, providerId: AIProviderId): {
        ...
        if (this.isProviderCircuitOpen(providerId)) {
            return { allowed: false, reason: `Provider ${providerId} circuit is OPEN` };
        }
    ...
    getStatus(): SafetyStatus {
        return {
            ...
            keyCircuits: Object.fromEntries(
                Array.from(this.state.keyCircuitBreakers.entries()).map(([k, v]) => [k, v.state])
            ),
            providerCircuits: Object.fromEntries(
                Array.from(this.state.providerCircuitBreakers.entries()).map(([k, v]) => [k, v.state])
            ),
        };
    }
