...
export class KeyRouter {
     ...
     */
    markRateLimited(keyId: string, providerId: AIProviderId, retryAfterMs: number): void {
        ...
        if (currentState?.promotedKeyId === keyId || !currentState?.promotedKeyId) {
            console.log(`[KeyRouter] Key ${keyId} rate-limited. Initiating automatic rotation...`);
    ...
    markHealthy(keyId: string, providerId: AIProviderId): void {
        ...
        if (!currentState?.promotedKeyId) {
            ...
            console.log(`[KeyRouter] Key ${keyId} promoted as primary for ${providerId}`);
            this.emitRotation({
                type: 'key_promoted',
                keyId,
                providerId
     ...
     */
    private checkRotationRecovery(providerId: AIProviderId, keys: KeyMetadata[]): void {
        ...
        const now = Date.now();
        const cooldownPassed = !stats.lastRateLimited ||
            (now - stats.lastRateLimited) >= this.config.rotationCooldown!;
        const notRateLimited = !stats.rateLimitedUntil || now >= stats.rateLimitedUntil;

        if (cooldownPassed && notRateLimited) {
            console.log(`[KeyRouter] Restoring ${rotatedOutKey.label} as primary for ${providerId} after cooldown`);
    ...
    selectKey(
        ...
        const healthyKeys = availableKeys.filter(k => !this.isKeyRateLimited(k.id));
        const rateLimitedKeys = availableKeys.filter(k => this.isKeyRateLimited(k.id));

        // Prefer healthy keys, but keep rate-limited as fallback
        if (healthyKeys.length > 0) {
            availableKeys = healthyKeys;
        } else {
            console.log(`[KeyRouter] All ${providerId} keys are rate-limited. Using least-restricted one.`);
        ...
        if (rotationState?.promotedKeyId) {
            const promotedKey = availableKeys.find(k => k.id === rotationState.promotedKeyId);
    ...
    private selectByPriority(keys: KeyMetadata[], modelId?: string): KeyMetadata {
        ...
        const statusMap = { valid: 1, testing: 2, untested: 3, retry_scheduled: 4, invalid: 5 };

        const sorted = [...keys].sort((a, b) => {
            // 1. Status Check (Valid > Testing > Untested)
            const sA = statusMap[a.verificationStatus || 'untested'];
            const sB = statusMap[b.verificationStatus || 'untested'];
            if (sA !== sB) return sA - sB;

            // 2. Key Priority (High > Medium > Low)
            const pA = priorityMap[a.priority || 'medium'];
            const pB = priorityMap[b.priority || 'medium'];
            if (pA !== pB) return pA - pB;

            // 3. Model Priority Tie-Breaker (User Preference per Model)
            if (modelId) {
                const metaA = a.verifiedModelsMeta?.find(m => m.modelId === modelId);
                const metaB = b.verifiedModelsMeta?.find(m => m.modelId === modelId);
                // Higher numerical value = higher priority (e.g. 5 is best)
                const mpA = metaA?.modelPriority || 0;
                const mpB = metaB?.modelPriority || 0;
                if (mpA !== mpB) return mpB - mpA; // Descending sort
            }

            // 3. Error Rate (Lower = Better)
            const statsA = this.getStats(a.id);
            const statsB = this.getStats(b.id);
            const errorRateA = statsA.requestCount > 0 ? statsA.errorCount / statsA.requestCount : 0;
            const errorRateB = statsB.requestCount > 0 ? statsB.errorCount / statsB.requestCount : 0;

            if (errorRateA !== errorRateB) return errorRateA - errorRateB;

            // 4. Verification Latency (Faster = Better)
            const lA = a.averageLatency || 9999;
            const lB = b.averageLatency || 9999;
            if (lA !== lB) return lA - lB;

            // 5. Load Balancing (Usage Count)
    ...
    private selectRoundRobin(keys: KeyMetadata[], providerId: string): KeyMetadata {
    ...
    private selectFailover(keys: KeyMetadata[], modelId?: string): KeyMetadata {
        // Failover logic can also respect priority/latency in the remaining pool
