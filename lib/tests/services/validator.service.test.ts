...
import { modelVerifier } from "../../services/validation/model-verifier";
import { availabilityManager } from "../../services/availability/availability.manager";

// Mock dependencies
vi.mock("../../services/vault/vault.service");
vi.mock("../../services/validation/model-verifier");
vi.mock("../../services/availability/availability.manager");
vi.mock("../../db", () => ({
    db: {
        modelCache: {
...
describe("ValidatorService", () => {
    ...
    it("should resume pending validations", async () => {
        vi.mocked(vaultService.listKeys).mockResolvedValue([
            { id: "p1", verificationStatus: "testing" },
            { id: "p2", verificationStatus: "untested" }
        ] as any);

        vi.mocked(vaultService.getKeyMetadata).mockResolvedValue({ id: "p1" } as any);
    ...
    it("should avoid double-queuing the same key", async () => {
        const keyId = "key1";
        vi.mocked(vaultService.getKeyMetadata).mockResolvedValue({ id: keyId } as any);
