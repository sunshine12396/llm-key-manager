...
import { KeyMetadata, VerifiedModelMetadata, useLLMKeyManager, availabilityManager } from "llm-key-manager";
...
export const KeyRow: React.FC<KeyRowProps> = ({
  ...
  // Fetch detailed model statuses on mount and when keyId changes or status updates
  useEffect(() => {
    if (!keyData.id) return;

    const fetchStatuses = async () => {
      const models = await availabilityManager.getModelsForKey(keyData.id);
    ...
    const unsubscribe = availabilityManager.onUpdate(fetchStatuses);
    return unsubscribe;
  }, [keyData.id]);
