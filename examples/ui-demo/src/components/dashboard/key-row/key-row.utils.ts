...
export function formatRetryTime(
  ...
  if (nextRetryMs <= 0) return "SOON";

  // Absolute time (e.g. 12:45)
  const timeStr = new Date(nextRetryAt).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });

  if (nextRetryMs < 60000) {
    return `${Math.ceil(nextRetryMs / 1000)}S (${timeStr})`;
  }
  return `${Math.ceil(nextRetryMs / 60000)}M (${timeStr})`;
}
