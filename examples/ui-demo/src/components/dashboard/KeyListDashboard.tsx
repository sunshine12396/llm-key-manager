...
import { KeyMetadata, useLLMKeyManager, safetyGuard } from "llm-key-manager";
...
export const KeyListDashboard: React.FC = () => {
  ...
  const handleResetSafety = () => {
    if (window.confirm("This will reset all circuit breakers and safety overrides. Continue?")) {
      safetyGuard.resetAll();
      window.location.reload(); // Hard reload to clear all service states
    }
  };
