...
import { vaultService, availabilityManager } from "llm-key-manager/services";
import { validatorService } from "llm-key-manager/services/validation/validator.service";
import { ValidationEvent } from "llm-key-manager/services/validation/validation.types";

import { backgroundJobs } from "llm-key-manager/lifecycle/background-jobs";
import {
  KeyMetadata,
  AIProviderId,
  KeyVerificationStatus,
} from "llm-key-manager/models/metadata";
...
const LLMKeyManagerContext = createContext<
  LLMKeyManagerContextType | undefined
>(undefined);

export const LLMKeyManagerProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  useEffect(() => {
    const unsubscribe = validatorService.subscribe((event: ValidationEvent) => {
      // Update events list
  ...
  const addKey = async (
    ...
    // 6. Update key status
    console.log(
      `[LLMKeyManagerProvider] Starting background validation for key ${id}...`,
    );
    ...
    validatorService.queueValidation(id, 2).catch((err: any) => {
...
export const useLLMKeyManager = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useLLMKeyManager must be used within an LLMKeyManagerProvider",
    );
  }
  return context;
};
