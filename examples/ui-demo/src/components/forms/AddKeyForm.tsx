...
import { AIProviderId, useLLMKeyManager } from "llm-key-manager";
...
import { getProviderAdapter } from "llm-key-manager/providers/provider.registry";
...
export const AddKeyModal: React.FC<AddKeyModalProps> = ({
  ...
  const formatValidation =
    key.length > 0 ? adapter.validateKeyFormat(key) : null;
  const isValidFormat = formatValidation?.isValid ?? false;
  const canSubmit =
    key.length > 0 && label.length > 0 && isValidFormat && !isSubmitting;
          ...
          <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-start gap-3 text-emerald-400 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div>
              ...
              <p className="text-xs opacity-80 mt-1">
                Background validation is now running. You'll be notified when
        ...
        {!addedSuccessfully && (
          <div className="p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl flex items-start gap-3 text-indigo-400">
            ...
            <p className="text-[11px] font-medium leading-relaxed">
              ...
              notified of available models automatically.
            </p>
          </div>
        )}
          ...
          <div>
            <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-2 block">
              <div className="flex justify-between items-center">
                ...
                {key.length > 0 && (
                  <Badge
                    variant={isValidFormat ? "emerald" : "amber"}
                    size="sm"
                  >
